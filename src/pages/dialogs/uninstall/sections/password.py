...
class PasswordSection(Section):
	def __init__(self):
		...
		title = QLabel("Enter the password to uninstall", objectName="input-title")
		...
		self.setDefaultButtons(prevLabel="Cancel", nextLabel="Uninstall")
	...
	def primaryAction(self):
		try:
			...
			run(["sudo", "-k"])
			run(["sudo", "-Sv"], input=passw.encode(), check=True)
